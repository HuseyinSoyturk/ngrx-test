...
export function bookReducer(state: BookState = initialBookState, action: BookActions.Actions): BookState {
    switch (action.type) {
        case BookActions.ADD_BOOK:
            const addedBooks: Book[] = [...state.books, action.payload];
            return {
                ...state,
                books: addedBooks,
        ...
        case BookActions.UPDATE_BOOK:
            ...
            const book = editedBooks.find(obj => obj.id === action.payload.id);
        ...
        case BookActions.DELETE_BOOK:
            const removedBooks: Book[] = state.books.filter(obj => obj.id !== action.payload);
            return {
                ...state,
                books: removedBooks
            };
