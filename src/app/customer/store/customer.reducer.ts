...
export function customerReducer(state: CustomerState = initialCustomerState, action: CustomerActions.Actions): CustomerState {
    switch (action.type) {
        case CustomerActions.ADD_CUSTOMER:
            const addedCustomer: Customer[] = [...state.customers, action.payload];
            return {
                ...state,
        ...
        case CustomerActions.EDIT_CUSTOMER:
            ...
            const customer = editedCustomer.find(obj => obj.id === action.payload.id);
        ...
        case CustomerActions.DELETE_CUSTOMER:
            const removedCustomers: Customer[] = state.customers.filter(obj => obj.id !== action.payload);
            return {
                ...state,
